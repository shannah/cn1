...
public final class GeneralPath implements Shape {
     ...
     * there may be many stroke settings, but only the most recently requested
     * one will remain in the cache. This is to try to maintain a constant
     * memory footprint. If performance tests reveal that it would be better to
     ...
     * This flag is set if any changes are made to the path.
     */
    private boolean dirty = true;

    /**
     * Gets an alpha mask for this path using the specified stroke settings. If
     * {@code stroke} is null, then this returns an alpha mask for the shape
     * filled.
     *
     * <p>
     * Alpha masks are platform-dependent - hence why it is returned as an
     * {@code Object}. Some platforms (e.g. iOS) use OpenGL texture IDs for
     * alpha masks for best performance. Other platforms might actually store an
     * array of bytes representing alpha values.</p>
     *
     * @param stroke The stroke to be used for generating the alpha mask. A
     * {@code null} stroke will cause a "fill" alpha mask to be returned.
     * @return The platform dependent stroke object, or null if the platform
     * doesn't supported alpha masks.
     *
