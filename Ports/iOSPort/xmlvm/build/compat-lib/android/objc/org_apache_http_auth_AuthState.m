#import "org_apache_http_auth_Credentials.h"
#import "android_internal_Assert.h"
#import "org_apache_http_auth_AuthScheme.h"

// Automatically generated by xmlvm2obj. Do not edit!


#import "org_apache_http_auth_AuthState.h"
#import "org_xmlvm_iphone_NSObject.h"


@implementation org_apache_http_auth_AuthState;

+ (void) initialize
{
    if (strcmp(class_getName(self), "org_apache_http_auth_AuthState") == 0) {
    }
}

- (id) init
{
    if (self = [super init]) {
    }
    return self;
}

- (void) dealloc
{
    if (strcmp(object_getClassName(self), "org_apache_http_auth_AuthState") == 0) {
        [self dealloc__];
    }
    [super dealloc];
}

- (void) __init_org_apache_http_auth_AuthState__
{
    XMLVMElem _r0;
    _r0.o = self;
    [_r0.o retain];
    [((java_lang_Object*) _r0.o) __init_java_lang_Object__];
    [_r0.o release];
    return;
}
- (org_apache_http_auth_AuthScheme*) getAuthScheme__
{
    XMLVMElem _r0;
    XMLVMElem _r1;
    _r1.o = self;
    _r0.o = JAVA_NULL;
    [_r1.o retain];
    [android_internal_Assert NOT_IMPLEMENTED__];
    _r0.o = JAVA_NULL;
    [_r1.o release];
    return _r0.o;
}
- (void) setAuthScheme___org_apache_http_auth_AuthScheme :(org_apache_http_auth_AuthScheme*)n1
{
    XMLVMElem _r0;
    XMLVMElem _r1;
    _r0.o = self;
    _r1.o = n1;
    [_r0.o retain];
    [_r1.o retain];
    [android_internal_Assert NOT_IMPLEMENTED__];
    [_r0.o release];
    [_r1.o release];
    return;
}
- (void) setCredentials___org_apache_http_auth_Credentials :(org_apache_http_auth_Credentials*)n1
{
    XMLVMElem _r0;
    XMLVMElem _r1;
    _r0.o = self;
    _r1.o = n1;
    [_r0.o retain];
    [_r1.o retain];
    [android_internal_Assert NOT_IMPLEMENTED__];
    [_r0.o release];
    [_r1.o release];
    return;
}

@end

