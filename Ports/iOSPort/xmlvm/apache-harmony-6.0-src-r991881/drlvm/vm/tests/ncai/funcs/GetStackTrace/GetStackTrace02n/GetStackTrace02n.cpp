...
const char test_case_name[] = "GetStackTrace02n";
...
Java_ncai_funcs_GetStackTrace02n_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetStackTrace02n_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetStackTrace02n_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetStackTrace02n_TestFunction1
  (JNIEnv *env, jobject obj)
{
...
bool CheckFrames(ncaiEnv *ncai_env, ncaiThread ncai_thread, ncaiFrameInfo** ret_frames, jint* ret_count)
{
    ...
    ncaiError ncai_err;
    jint frame_count = 100;
    jint ret = 100;
    bool ret_val = true;

    fprintf(stderr, "calling ncai->GetStackTrace()...\n");
    ncai_err = ncai_env->GetStackTrace(ncai_thread, frame_count, *ret_frames, &ret);
    if (ncai_err != NCAI_ERROR_THREAD_NOT_ALIVE)
    {
        ret_val = false;
        fprintf(stderr, "ncai->GetStackTrace() has return error %d, must be %d\n",
            ncai_err, NCAI_ERROR_THREAD_NOT_ALIVE);
    }
