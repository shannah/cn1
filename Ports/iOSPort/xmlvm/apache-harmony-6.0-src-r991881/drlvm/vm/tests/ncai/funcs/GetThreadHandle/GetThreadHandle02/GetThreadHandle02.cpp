...
static const char test_case_name[] = "GetThreadHandle02";
...
Java_ncai_funcs_GetThreadHandle02_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetThreadHandle02_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetThreadHandle02_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetThreadHandle02_TestFunction1
  (JNIEnv *env, jobject obj)
{
                               ...
                               jmethodID catch_method, jlocation catch_location)
{
    ...
    //////////////////////////
    test = false;
    ncai_err = ncai_env->GetAllThreads(&returned, &threads);
    if (ncai_err != NCAI_ERROR_NONE)
    {
        fprintf(stderr, "ncai->GetAllThreads() returned error: %d", ncai_err);
        test = false;
        g_stop_thread = 1;
        return;
    }
    ncaiThreadInfo info1;
    ncai_err = ncai_env->GetThreadInfo(ncai_thread, &info1);
    if (ncai_err != NCAI_ERROR_NONE)
    {
        fprintf(stderr, "ncai->GetThreadInfo() returned error: %d", ncai_err);
        test = false;
        g_stop_thread = 1;
        return;
    }
    ...
    if (ncai_err != NCAI_ERROR_NONE)
    {
        fprintf(stderr, "ncai->SuspendThread() returned error: %d", ncai_err);
        test = false;
        g_stop_thread = 1;
        return;
    }
    ...
    {
        ncaiThreadInfo info2;
        ncai_err = ncai_env->GetThreadInfo(threads[i], &info2);
        if (ncai_err != NCAI_ERROR_NONE)
        {
            fprintf(stderr, "ncai->GetThreadInfo() returned error: %d", ncai_err);
            test = false;
        ...
        if(info1.kind == info2.kind && strcmp(info1.name, info2.name) == 0)
        {
            test = true;
            fprintf(stderr, "\nMATCH with '%s(%d)'", info2.name, info2. kind);
        }
    }
    ...
    if (ncai_err != NCAI_ERROR_NONE)
    {
        fprintf(stderr, "ncai->ResumeThread() returned error: %d", ncai_err);
        test = false;
        g_stop_thread = 1;
        return;
    }
