...
const char test_case_name[] = "GetStackTrace04";
...
Java_ncai_funcs_GetStackTrace04_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetStackTrace04_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetStackTrace04_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetStackTrace04_TestFunction1
  (JNIEnv *env, jobject obj)
{
                               ...
                               jmethodID catch_method, jlocation catch_location)
{
    ...
    if(frames[frames_returned - 1].return_address != NULL)
    {
    ...
    test = test && CheckFrames(ncai_env, ncai_thread, &frames, &frames_returned);
    if(frames[frames_returned - 1].return_address != NULL)
    {
        fprintf(stderr, "\nincorrect frames count\n");
        test = false;
