 ...
 *
 */
public class GetThreadInfo02n extends Thread{
    ...
    static {
        try{
            System.loadLibrary("GetThreadInfo02n");
        }
        catch(Throwable e){
            NoLibrary = true;
        }
    ...
    static public void main(String args[]) {
        if(NoLibrary) return;
        new GetThreadInfo02n("java_thread1").start();
        new GetThreadInfo02n("java_thread2").start();
        new GetThreadInfo02n("java_thread3").start();
        special_method();
        return;
    }
