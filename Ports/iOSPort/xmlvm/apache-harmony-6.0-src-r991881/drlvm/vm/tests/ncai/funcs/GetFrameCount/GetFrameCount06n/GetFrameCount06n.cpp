...
const char test_case_name[] = "GetFrameCount06n";
...
Java_ncai_funcs_GetFrameCount06n_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetFrameCount06n_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount06n_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount06n_TestFunction1
  (JNIEnv *env, jobject obj)
{
...
bool CheckFrames(ncaiEnv *ncai_env, ncaiThread ncai_thread, ncaiFrameInfo** ret_frames, jint* ret_count)
{
    ...
    ncai_err = ncai_env->GetStackTrace(ncai_thread, -1, frames, &frames_returned);
    if (ncai_err != NCAI_ERROR_ILLEGAL_ARGUMENT)
    {
        fprintf(stderr, "ncai->GetStackTrace() returned: %d, must be %d\n",
            ncai_err, NCAI_ERROR_ILLEGAL_ARGUMENT);
        ret = false;
    }
