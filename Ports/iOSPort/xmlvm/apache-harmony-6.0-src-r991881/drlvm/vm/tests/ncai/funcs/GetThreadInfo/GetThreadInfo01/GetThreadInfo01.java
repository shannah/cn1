 ...
 *
 */
public class GetThreadInfo01 extends Thread{
    ...
    static {
        try{
            System.loadLibrary("GetThreadInfo01");
        }
        catch(Throwable e){
            NoLibrary = true;
        }
    ...
    static public void main(String args[]) {
        if(NoLibrary) return;
        new GetThreadInfo01("java_thread1").start();
        new GetThreadInfo01("java_thread2").start();
        new GetThreadInfo01("java_thread3").start();
        special_method();
        return;
    }
