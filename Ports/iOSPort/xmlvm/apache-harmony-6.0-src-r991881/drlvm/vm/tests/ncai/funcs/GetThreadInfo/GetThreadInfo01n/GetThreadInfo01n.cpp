...
const char test_case_name[] = "GetThreadInfo01n";
/*
...
Java_ncai_funcs_GetThreadInfo01n_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetThreadInfo01n_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetThreadInfo01n_TestFunction1
...
*/
    ...
    {
        ncai_err = ncai_env->GetThreadInfo(threads[h], NULL);
        if (ncai_err != NCAI_ERROR_NULL_POINTER)
        {
            fprintf(stderr, "ncai->GetThreadInfo() returned invalid error: %d,\n", ncai_err);
            test = false;
            g_stop_thread = 1;
            return;
        }
