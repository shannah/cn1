 ...
 *
 */
public class GetThreadHandle01n extends Thread{
    ...
    static {
        try{
            System.loadLibrary("GetThreadHandle01n");
        }
        catch(Throwable e){
            NoLibrary = true;
        }
    ...
    GetThreadHandle01n(String name)
    {
        super(name);
    }
    ...
    static public void main(String args[]) {
        if(NoLibrary) return;
        new GetThreadHandle01n("java_thread").start();
        special_method();
        /*
        while(!stopsignal()){
            try {
