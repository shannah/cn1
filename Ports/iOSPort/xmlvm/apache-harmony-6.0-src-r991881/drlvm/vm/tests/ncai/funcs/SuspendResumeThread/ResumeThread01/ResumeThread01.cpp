...
const char test_case_name[] = "ResumeThread01";
...
Java_ncai_funcs_ResumeThread01_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
extern "C" JNIEXPORT void JNICALL
Java_ncai_funcs_ResumeThread01_increment(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
Java_ncai_funcs_ResumeThread01_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_ResumeThread01_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_ResumeThread01_TestFunction1
  (JNIEnv *env, jobject obj)
{
                               ...
                               jmethodID catch_method, jlocation catch_location)
{
    ...
    test = test && CheckResume(ncai_env, ncai_thread);
...
bool CheckResume(ncaiEnv *ncai_env, ncaiThread ncai_thread)
{
    ...
    for (int i = 0; i < 40; i++)
    {
        ...
        if (ncai_err == NCAI_ERROR_NONE)
        {
            fprintf(stderr, "ncai->ResumeThread() returned wrong error: %d,\n", ncai_err);
            ret = false;
            break;
        }
    }
