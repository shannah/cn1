...
const char test_case_name[] = "GetFrameCount03n";
...
Java_ncai_funcs_GetFrameCount03n_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetFrameCount03n_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount03n_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount03n_TestFunction1
  (JNIEnv *env, jobject obj)
{
...
bool CheckFrames(ncaiEnv *ncai_env, ncaiThread ncai_thread, ncaiFrameInfo** ret_frames, jint* ret_count)
{
    ...
    ncai_err = ncai_env->GetFrameCount(ncai_thread, NULL);
