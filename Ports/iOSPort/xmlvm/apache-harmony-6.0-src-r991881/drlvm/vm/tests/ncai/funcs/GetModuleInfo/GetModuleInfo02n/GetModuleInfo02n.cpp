...
const char test_case_name[] = "GetModuleInfo02n";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    for (j = 0; j < tcount; j++)
    {
        ncai_err = ncai_env->GetModuleInfo(modules[j], NULL);
        if (ncai_err == NCAI_ERROR_NONE)
        {
            fprintf(stderr, "ncai_env->GetModuleInfo() didn't return an error");
            test = false;
            return;
        }
        fprintf(stderr, "ncai_env->GetModuleInfo() returned an error: %d\n", ncai_err);
    }
