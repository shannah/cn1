 ...
 *
 */
public class GetThreadHandle02n extends Thread{
    ...
    static {
        try{
            System.loadLibrary("GetThreadHandle02n");
        }
        catch(Throwable e){
            NoLibrary = true;
        }
    ...
    GetThreadHandle02n(String name)
    {
        super(name);
    }
    ...
    static public void main(String args[]) {
        if(NoLibrary) return;
        new GetThreadHandle02n("java_thread").start();
        special_method();
        /*
        while(!stopsignal()){
            try {
