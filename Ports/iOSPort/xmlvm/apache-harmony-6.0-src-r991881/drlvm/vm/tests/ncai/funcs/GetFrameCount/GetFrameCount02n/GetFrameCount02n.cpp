...
const char test_case_name[] = "GetFrameCount02n";
...
Java_ncai_funcs_GetFrameCount02n_resumeagent(JNIEnv *env, jclass cls)
{
    //warning fix
...
Java_ncai_funcs_GetFrameCount02n_stopsignal(JNIEnv *env, jclass cls)
{
    //warning fix
    int w_fix = sizeof(cls);
    w_fix += sizeof(env);
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount02n_TestFunction
...
extern "C" JNIEXPORT void JNICALL Java_ncai_funcs_GetFrameCount02n_TestFunction1
  (JNIEnv *env, jobject obj)
{
...
bool CheckFrames(ncaiEnv *ncai_env, ncaiThread ncai_thread, ncaiFrameInfo** ret_frames, jint* ret_count)
{
    ...
    fprintf(stderr, "ncai->GetFrameCount() incorrectly returned code %d\n", ncai_err);
    return false;
}
