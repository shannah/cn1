...
const char test_case_name[] = "ReadMemory02";
static char buf[] = "1234567890qwerty";
...
void JNICALL callbackException(prms_EXCPT)
{
    ...
    ncai_err = ncai_env->ReadMemory(&buf, 16, &out_buf);
    if(ncai_err != NCAI_ERROR_NONE)
    {
        fprintf(stderr, "ReadMemory returned error: %d\n", ncai_err);
        test = false;
        return;
    }
